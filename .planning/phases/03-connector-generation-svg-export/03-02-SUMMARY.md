...
plan: 02
subsystem: core
tags: [svg, laser-cutter, kerf, bezier, kurbo, wasm, export]

# Dependency graph
requires:
  - phase: 03-connector-generation-svg-export
    provides: ClassicKnobConnector with bezier curves on all internal edges
  - phase: 02-grid-engine-data-model
    provides: PuzzleGrid shared-edge model, PuzzleConfig, Edge struct
provides:
...
tech-stack:
  added: []
  patterns: [single-path-svg, edge-local-to-global-affine-transform, polyline-offset-kerf, quarter-arc-rounded-corners]
